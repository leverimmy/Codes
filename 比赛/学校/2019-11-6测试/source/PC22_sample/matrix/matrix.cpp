...
int n,m,s,f[402][402][402],ans;
int v[402][402],w[402][402];
signed main()
{
	fin("matrix.in"),fout("matrix.out");
	n=rd ui(),m=rd ui(),s=rd ui();
	f1(i,n)
		f1(j,m)
			v[i][j]=rd ui();
	f1(i,n)
	...
	f1(i,n)
		f1(j,m)
			f0(k,s+1)
				if(v[i][j]>k)
					f[i][j][k]=mx(f[i-1][j][k],f[i][j-1][k]);
				else
					f[i][j][k]=mx(mx(f[i-1][j][k],f[i][j-1][k]),w[i][j]+mx(f[i-1][j][k-v[i][j]],f[i][j-1][k-v[i][j]]));
	f0(i,s+1)
		ans=max(ans,f[n][m][i]);
	wr ui(ans);
	return 0;
